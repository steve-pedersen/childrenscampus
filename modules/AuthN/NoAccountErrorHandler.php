 ...
 */
class Ccheckin_AuthN_NoAccountErrorHandler extends Ccheckin_Master_ErrorHandler
{
    ...
    protected function getTemplateFile () { return 'noAccount.html.tpl'; }
    ...
    protected function handleError ($error)
    {
        ...
        if ($identity->getAuthenticated())
        {
            ...
            if (($allowCreateAccount = $identity->getProperty('allowCreateAccount')))
            {
                ...
                if ($simsAccount = $importer->findAccount($identity->getProperty('username')))
                {
                    $account->username = $simsAccount->username;
                    $account->emailAddress = $identity->getProperty('emailAddress');
                    $account->firstName = $simsAccount->firstName;
                    $account->lastName = $simsAccount->lastName;
                    $account->createdDate = new DateTime;
                    $account->save();
                    
                    $authZ = $this->getApplication()->authorizationManager;
                    
                    if ($affiliations = $this->getAffiliations($identity))
                    {
                        $faculty = (in_array('employee faculty', $affiliations) || 
                                    in_array('employee staff', $affiliations));
                    }

                    
                    if (!$faculty && ($enrollments = $importer->findEnrollments($account->username)))
                    {
                        $siteSettings = $this->getApplication()->siteSettings;
                        try {
                            if (($currentSemesterId = $semesterManager->getCurrentSemesterCode())) // ***needs currentSemesterCode
                            {
                                $currentSemester = $semesterManager->getCurrentSemester();
                                if (($courses = $currentSemester->findCoursesByEnrollments($enrollments)))
                                {
                                ...
                                if (($courses = $currentSemester->findCoursesByEnrollments($enrollments, 'instructor'))) {
                                    foreach ($courses as $course)
                                    {
                                        $faculty = true;
                                    }
                ...
                $roleNames[] = ($faculty ? 'Teacher' : 'Student');
                $roleNames = array_unique($roleNames);
                
                if (!empty($roleNames))
                {
                    $roleRecords = $roles->find($roles->name->inList($roleNames));
